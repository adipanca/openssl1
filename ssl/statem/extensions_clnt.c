...
static int add_key_share(SSL *s, WPACKET *pkt, unsigned int curve_id)
{
    ...
    if (s->s3->tmp.pkey != NULL) {
      ...
      if (!do_pqc) {
        // fprintf(stderr, "[cli] HYBRID if (!do_pqc) {\n");
    ...
    //   fprintf(stderr, "[cli] HYBRID if (!do_pqc) { classical_encodedlen\n");
    ...
    //   fprintf(stderr, "[cli] HYBRID if (do_hybrid) { uint16_t encodedlen16;\n");
    ...
    //   fprintf(stderr, "[cli] HYBRID if (do_pqc) { encoded_point = oqs_encoded_point;\n");
      encoded_point = oqs_encoded_point;
      encodedlen = oqs_encodedlen;
    } else {
    //   fprintf(stderr, "[cli] HYBRID } else { encoded_point = oqs_encoded_point;\n");
      encoded_point = classical_encoded_point;
      encodedlen = classical_encodedlen;
    }

    // fprintf(stderr, "[cli] HYBRID header(be): %02x %02x %02x %02x (tot=%zu)\n", encoded_point[0], encoded_point[1], encoded_point[2], encoded_point[3], encodedlen);
